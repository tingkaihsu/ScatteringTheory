...
\begin{equation}
    \begin{split}
        \left[\delta_{V}^{(3)}(\mathbf{P}_{\alpha}-\mathbf{P}_{\beta})\right]^2 = \frac{V}{(2\pi)^3}\int_{V}{d^3x\,e^{i(\mathbf{P}_{\alpha}-\mathbf{P}_{\beta})\cdot\mathbf{x}}}\\
        \left[\delta_{T}(E_{\alpha}-E_{\beta})\right]^2 = \frac{T}{2\pi}\int_{T}{dt\,e^{i(E_{\alpha}-E_{\beta})t}}
    \end{split}
\end{equation}
...
\[\braket*{\Phi_{\mathbf{x}}}{\Phi_{\mathbf{p}}} = e^{i\mathbf{p}\cdot\mathbf{x}/\hbar}\]
we take it to be normalized so that the integral of its absolute-value squared \textit{over the box} is unity:
\begin{equation}
    \braket*{\Phi_{\mathbf{x}}}{\Phi_{\mathbf{p}}^{\text{Box}}} = \frac{e^{i\mathbf{p}\cdot\mathbf{x}/\hbar}}{\sqrt{V}}
\end{equation}
