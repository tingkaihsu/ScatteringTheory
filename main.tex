...
\[\psi_{g}(\mathbf{x}, t)\rightarrow\frac{1}{r}\int{d^2k_{\perp}}\int_{-\infty}^{\infty}{dk_3\,g(\mathbf{k}_{\perp}, k_3)}\]
\begin{equation}
...
That is, we assume that the function $g(\mathbf{k}_{\perp}, k_3)$, though smooth, is strongly peaked at $k_3=k_0$ and $\mathbf{k}_{\perp} = 0$, so we could set $k_3$ in $g(\mathbf{k}_{\perp}, k_3)$ equal to the value $k_3\mu r/\hbar t$, so that,
\[
    \psi_{g}(\mathbf{x}, t)\rightarrow\frac{1}{r}f_{\mathbf{k}_0}(\hat{x})\int{d^2k_{\perp}\,g(\mathbf{k}_{\perp}, \mur/\hbar t)}
\]
\[\times\int_{-\infty}^{\infty}{dk_3\,\exp(ik_3r-i\hbar tk_3^2\mu)}\]
\end{document}